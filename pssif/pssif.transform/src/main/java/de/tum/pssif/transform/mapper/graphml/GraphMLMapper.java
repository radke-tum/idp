...
public abstract class GraphMLMapper implements Mapper {
  ...
  private static void readAnnotations(ElementType type, Element element, GraphMLElement inElement) {
  ...
  protected void writeInternal(Metamodel metamodel, Model model, OutputStream outputStream, boolean writeAnnotations) {
  ...
  private GraphMlContext addNodesToGraph(GraphMLGraph graph, NodeTypeBase nodeType, Model model, GraphMlContext context) {
    Attribute idAttribute = nodeType.getAttribute(PSSIFConstants.BUILTIN_ATTRIBUTE_ID).getOne();
    context.registerNodeAttribute(new GraphMlAttrImpl(idAttribute.getName(), idAttribute.getType().getName()));
    for (Node pssifNode : nodeType.apply(model, false).getMany()) {
      ...
      for (Attribute attr : nodeType.getAttributes()) {
        ...
        context.registerNodeAttribute(new GraphMlAttrImpl(attr.getName(), attr.getType().getName()));
        if (!idAttribute.equals(attr) && val.isOne()) {
          node.setValue(attr.getName(), val.getOne().getValue().toString());
        }
      }
      for (Entry<String, String> annotation : pssifNode.getAnnotations().getMany()) {
        context.registerNodeAttribute(new GraphMlAttrImpl(annotation.getKey(), GraphMLTokens.STRING, GraphMLTokens.ANNOTATION));
    ...
    for (ConnectionMapping mapping : edgeType.getMappings().getMany()) {
      ...
      for (Edge pssifEdge : edges.getMany()) {
        ...
        for (Attribute attr : edgeType.getAttributes()) {
          ...
          context.registerEdgeAttribute(new GraphMlAttrImpl(attr.getName(), attr.getType().getName()));
          if (!idAttribute.equals(attr) && val.isOne()) {
            edge.setValue(attr.getName(), val.getOne().getValue().toString());
          }
        }
        for (Entry<String, String> annotation : pssifEdge.getAnnotations().getMany()) {
          context.registerEdgeAttribute(new GraphMlAttrImpl(annotation.getKey(), GraphMLTokens.STRING, GraphMLTokens.ANNOTATION));
          edge.setAnnotations(annotation.getKey(), annotation.getValue());
        }
        graph.addEdge(edge);
      }
    }
