...
public class MyModelContainer {
	...
	/**
	 * Add a new JunctionNode which was created through the Gui
	 * 
	 * @param nodeName
	 *            The name of the new Node
	 * @param type
	 *            The type of the new Node
	 */
	public MyJunctionNode addNewJunctionNodeFromGUI(String nodeName, MyJunctionNodeType type) {
		JunctionNodeType nodeType = meta.getJunctionNodeType(type.getName()).getOne();
		...
		nodeType.getAttribute(PSSIFConstants.BUILTIN_ATTRIBUTE_ID)
				...
				.set(newNode,
						PSSIFOption.one(PSSIFValue.create(nodeName
								+ newNodeIdCounter++)));
		MyJunctionNode newMyNode = new MyJunctionNode(newNode, type);
		// TODO added
		DBMapperImpl.changedJunctionNodes.add(newMyNode);
		junctionNodes.add(newMyNode);
		return newMyNode;
	}
	...
	public boolean addNewEdgeGUI(MyNode source, MyNode destination,
		...
		final String preIDString = "edge";
		int max = 0;
		for (MyEdge edge: ModelBuilder.getAllEdges()){
			String id = edge.getEdge().getId(); 
			if (id.startsWith(preIDString))
			{
				 id = id.substring(preIDString.length());
				 try{
		...
		if (mapping != null) {
			...
			PSSIFOption<PSSIFValue> id = PSSIFOption.one(PSSIFValue
					.create(preIDString + newEdgeIdCounter));
			edgetype.getType()
