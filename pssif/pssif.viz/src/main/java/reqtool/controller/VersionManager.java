...
public class VersionManager {
	...
	public boolean createNewVersion() {
		try {
			...
			Double nv = Double.parseDouble(oldVersion.getOne().asString()) + 1;
			createNewVersionNode(String.valueOf(nv));
	...
	public void createNewVersionNode(String newVersion) {
		...
		String id = selectedNode.getNode().getId();
		newVersionNode.getNodeType().getType().getAttribute(PSSIFConstants.BUILTIN_ATTRIBUTE_ID).getOne().set(newVersionNode.getNode(), PSSIFOption.one(PSSIFValue.create(id)));
		String name = selectedNode.getName(); 
		if (name.contains("("))
			name = name.substring(0, name.indexOf("("));
		newVersionNode.getNodeType().getType().getAttribute(PSSIFConstants.BUILTIN_ATTRIBUTE_NAME).getOne().set(newVersionNode.getNode(), PSSIFOption.one(PSSIFValue.create(name + "(" +(new Timestamp(date.getTime())+")"))));
		String oldid = oldVersion.getOne().asString();
		selectedNode.getNodeType().getType().getAttribute(PSSIFConstants.BUILTIN_ATTRIBUTE_ID).getOne().set(selectedNode.getNode(), PSSIFOption.one(PSSIFValue.create(id
				+ "_" + oldid)));
