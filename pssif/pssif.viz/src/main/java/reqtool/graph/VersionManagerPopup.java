...
public class VersionManagerPopup {
	...
	public static boolean showPopup(MyNode node) {
		...
		while (iterator.hasNext()) {
			...
			if (!attr.getName().equals(PSSIFConstants.BUILTIN_ATTRIBUTE_ID) && !attr.getName().equals(PSSIFConstants.BUILTIN_ATTRIBUTE_NAME) && !attr.getName().equals(PSSIFConstants.BUILTIN_ATTRIBUTE_GLOBAL_ID)
					 && !attr.getName().equals(PSSIFConstants.BUILTIN_ATTRIBUTE_VERSION)){
		...
		boolean nonempty_attrs=false;
    	for (Entry<Attribute, JTextField> input : attrTextFields.entrySet()) {
        	if ( input.getValue().getText()!=null && input.getValue().getText().length()>0)
        		nonempty_attrs =true;
    	}
    	
    	if(nonempty_attrs){
        	VersionManager vm = new VersionManager(node);
    		return vm.createNewVersion();
    	}
    	return false;
	}
