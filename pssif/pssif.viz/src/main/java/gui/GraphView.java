...
public class GraphView {
	...
	public GraphVisualization getGraphVisualization(){return graph;}
	
	
  /**
   * Create a new instance of a GraphView
   */
	public GraphView(MainFrame mainFrame, String importPath) {
	    ...
	    if (nodeDetails != null && nodeDetails.isSelected())
	    	details = true;
	  
	    graph = new GraphVisualization(new Dimension(x, y), details, importPath);
  ...
  public JPanel getGraphPanel() {
	...
	listOfInstances.addMouseListener(new MouseListener() {
		...
		public void mousePressed(MouseEvent e) {
			...
			JList<String> instanceList = (JList<String>)e.getSource();
			
			LinkedList<MyNode> graphNodes = new LinkedList<MyNode>();
			graphNodes = ModelBuilder.getAllNodes() ;
			
			for(MyNode mn: graphNodes){
				if(mn.getRealName().equals(instanceList.getSelectedValue()))
				{
					getGraph().vv.getPickedVertexState().pick(mn, true);
				}
				else
					getGraph().vv.getPickedVertexState().pick(mn, false);
			}
			getGraph().vv.repaint();
		}
  ...
  private JPanel addBasicOperationPanel() {
    ...
    collapseExpand = new JButton("Hide/Show Containment");
    ...
    collapseExpand.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        if (graph.isExpandable()) {
          ...
          collapseExpand.setText("Hide Containment");
        }

        if (graph.isCollapsable()) {
          graph.collapseNode();
          collapseExpand.setText("Show Containment");
        }
      }
    });
    ypos++;
...
private JTable createEdgeAttributTable() {
  ...
  private void updateNodeSidebar(String nodeName, MyNodeType nodeType, LinkedList<LinkedList<String>> attributes) {
    ...
    if (!graph.isCollapsable() && !graph.isExpandable()) {
      collapseExpand.setText("Hide/Show Containment");
      collapseExpand.setEnabled(false);
    }
    else {
