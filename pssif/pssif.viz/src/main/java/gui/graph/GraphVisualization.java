...
import gui.mouse.EnhancedModalGraphMouse;
import gui.mouse.MyPopupGraphMousePlugin;
...
public class GraphVisualization
{
	...
	public VisualizationViewer<IMyNode, MyEdge> vv;
	public Layout<IMyNode, MyEdge> layout;
	public VertexStrokeHighlight<IMyNode, MyEdge> vsh;
	...
	private HashMap<MyEdgeType, LineSpec> edgeLineMapping;
	...
	private NodeLocationWriterReader nodeLocationWriterReader;


	public GraphVisualization(Dimension d, boolean details, String importPath)
	{ 
		int dimension_size = 1000;
		...
		nodeLocationWriterReader = new NodeLocationWriterReader();
		...
		this.edgeLineMapping  = new HashMap<MyEdgeType, LineSpec>();
		...
		if (d != null)
		{
			...
			dimTmp = new Dimension(x, y);
		}
		else
			dimTmp = new Dimension(dimension_size, dimension_size);

		
		//load node positions if available
		layout = nodeLocationWriterReader.loadNodesFromFile(importPath, g, dimTmp);
		if (layout == null)
		{
			this.layout = new FRLayout<IMyNode, MyEdge>(this.g);
			layout.setGraph(g);
			this.layout.setSize(dimTmp);
		}
		...
		if (d != null) 
			...
			this.vv.setPreferredSize(new Dimension(dimension_size, dimension_size));
	...
	private void createEdgeTransformers()
	{
		...
		Transformer<MyEdge, Stroke> edgeTransformer = new Transformer<MyEdge, Stroke>()
			...
			{
				LineSpec lspec = edgeLineMapping.get(edge.getEdgeType());
				if (lspec == null)
				{
				...
				{
					return new BasicStroke(lspec.getThickness(), 0, 2, 0.0F, lspec.getType(), 0.0F);
				}
			}
				};
	...
	public Graph<IMyNode, MyEdge> getGraph() {
		return g;
	}
	...
	public HashMap<MyEdgeType, LineSpec> getEdgeLineMapping()
	{
		return this.edgeLineMapping;
	}
	...
	public void setEdgeLineMapping(HashMap<MyEdgeType, LineSpec> lineMapper) {
		this.edgeLineMapping = lineMapper;
		//this.nodeIconConfigWriterReader.setIcons(nodeIconMapping);
		updateGraph();
