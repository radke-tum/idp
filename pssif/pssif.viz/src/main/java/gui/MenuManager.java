...
import gui.graph.EdgeLinePopup;
...
public class MenuManager {
	...
	private JMenuBar addFileMenu(JMenuBar menuBar) {
		...
		edgeLine = new JMenuItem("Choose Edge Line");
		edgeLine.addActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
				EdgeLinePopup popup = new EdgeLinePopup(graphView);
				popup.showPopup();
			}
		});

		menuBar.getMenu(0).add(edgeLine);
	...
	*/
	...
	private JMenu addChangeLayouts() {
		...
		JMenuItem testIt = new JMenuItem("Test");
		testIt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (MyNode mn : ModelBuilder.getAllNodes())
				{
					...
					msg += mn.getName() + graphView.getGraph().layout.transform(mn).getX() + 
							" - "+ graphView.getGraph().layout.transform(mn).getY();
					msg += "\n";
				}
