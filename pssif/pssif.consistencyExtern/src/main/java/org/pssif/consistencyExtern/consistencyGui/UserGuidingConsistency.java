...
public class UserGuidingConsistency {
	...
	private static void openChooseTresholdsPopup() {
		...
		SpinnerModel synNumericModel = new SpinnerNumberModel(0, 0, 1, 0.1);
		final JSpinner synTresh = new JSpinner(synNumericModel);
		JComponent field = ((JSpinner.DefaultEditor) synTresh.getEditor());
		Dimension prefSize = field.getPreferredSize();
		prefSize = new Dimension(20, prefSize.height);
		field.setPreferredSize(prefSize);
		
		final JLabel labelSem = new JLabel("Semantic Threshold:");
		SpinnerModel semNumericModel = new SpinnerNumberModel(0, 0, 1, 0.1);
		final JSpinner semTresh = new JSpinner(semNumericModel);
		JComponent field2 = ((JSpinner.DefaultEditor) semTresh.getEditor());
		field2.setPreferredSize(prefSize);
		

		final JLabel labelCon = new JLabel("Context Threshold:");
		SpinnerModel conNumericModel = new SpinnerNumberModel(0, 0, 1, 0.1);
		final JSpinner conTresh = new JSpinner(conNumericModel);
		JComponent field3 = ((JSpinner.DefaultEditor) conTresh.getEditor());
		field3.setPreferredSize(prefSize);
		...
		buttonApply.addActionListener(new ActionListener() {
			...
			public void actionPerformed(final ActionEvent e) {
				...
				try{
					synTreshold = (double) synTresh.getValue();
					semTreshold = (double) semTresh.getValue();
					conTreshold = (double) conTresh.getValue();
				} catch(Exception ee){valuesAreDouble = false;}

				if (valuesAreDouble) {
					dialog.setVisible(false);
					ConsistencyData.initThresholds(synTreshold, semTreshold, conTreshold);
				} else {
					final JPanel tmpPanel = new JPanel();
				    JOptionPane.showMessageDialog(tmpPanel, "Thresholds must be between 0 and 1!", "Invalid Values", JOptionPane.ERROR_MESSAGE);
	...
	private static boolean changeIncreasing()
	{
	...
	private static void openChooseMergeCandidatesPopup() {
		...
		final MatchCandidateTableModel tableModel = new MatchCandidateTableModel(
				ConsistencyData.getInstance().getEqualsCandidateList());

		final JTable mergeTable = new JTable(tableModel);
		//tc.setHeaderRenderer(new CheckBoxHeader(new MyItemListener()));
		for (int i=0; i < mergeTable.getColumnCount(); i++)
		{
			final int c = i;
			String caption = mergeTable.getColumnName(i);
			mergeTable.getColumnModel().getColumn(i).setHeaderRenderer(new ButtonTableHeader(caption, new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					boolean inc = changeIncreasing();
					tableModel.sort(c, inc);
				}
			}));
		}
	...
	private static List<MatchMethod> chooseMatchingMethodsPopup(
		...
		methodTable.getColumnModel().getColumn(1).setMinWidth(125);
		methodTable.getColumnModel().getColumn(1).setMaxWidth(125);
		methodTable.getColumnModel().getColumn(2).setCellEditor(new SpinnerEditor());
		methodTable.getColumnModel().getColumn(2).setMaxWidth(50);
		methodTable.getColumnModel().getColumn(3).setMaxWidth(50);
		...
		JButton buttonApply = new JButton("Apply Selected Merge Methods");
		buttonApply.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent e) {
				MergeStatus status = chosenMethodsAreValid(methods);
				if (status == MergeStatus.NO_ERROR) {
					dialog.setVisible(false);
				} else
					JOptionPane.showMessageDialog(new JPanel(), "Invalid Configuration: " + getErroMessage(status), "Invalid Values", JOptionPane.ERROR_MESSAGE);
	 ...
	 *         [0..1])
	 */
	protected static MergeStatus chosenMethodsAreValid(List<MatchMethod> methods) {
		...
		if (!atLeastOneMetricSelected)
			return MergeStatus.AT_LEAST_ONE_SELECTED;
		if (!(syntacticWeight >= 0) || !(syntacticWeight <= 1))
			return MergeStatus.SYNTACTIC_WEIGHT;
		if (!(semanticWeight >= 0) || !(semanticWeight <= 1))
			return MergeStatus.SEMANTIC_WEIGHT;
		if (!(contextualWeight >= 0) || !(contextualWeight <= 1))
			return MergeStatus.CONTEXTUAL_WEIGHT;
		return MergeStatus.NO_ERROR;
	}

	private static String getErroMessage(MergeStatus ms)
	{
		switch (ms) {
		case AT_LEAST_ONE_SELECTED:
			return "At least one method should be selected!";
		case SEMANTIC_WEIGHT:
			return "Semantic methods' weight is more than 1!";
		case SYNTACTIC_WEIGHT:
			return "Syntactic methods' weight is more than 1!";
		case CONTEXTUAL_WEIGHT:
			return "Contextual methods' weight is more than 1!";
		case NO_ERROR:
			return "";
		default:
			return "";
