...
\paragraph{Deinstantify} This transformation results in a viewpoint hiding all instances of a certain ElementType within a model. It is required to, in contrast to the hide transformation (see below), allow subtypes of the deinstantified type to be instantiated, while still hiding the instances of the deinstantified type.
...
% In order to allow a metamodel element being mapped to different viewpoint elements, the alias transformation 

% \begin{itemize}
% \item AliasedNodeType: This operator is an alias for an existing NodeType. Instances are created as instances of the existing NodeType and annotated with the name of the operator. When gathering instances from a model, only the annotated instances are taken into account.
% \item ArtificializingNodeType: When creating a node, this operator additionally creates an artificial node and relates it to the created node through a specified ConnectionMapping.
% \item ArtificializingConnectionMapping: When creating an edge between two nodes, this operator additionally creates an artificial edge between the same two nodes.
% \item DeinstantifiedNodeType: This operator removes all instances of a specific NodeType from the view. Subtypes may still have instances.
% \item DeinstantifiedConnectionMapping: This operator hides all instances of a specific ConnectionMapping from the view.
% \item DelegatingEdgeType: This operator combines ConnectionMappings of multiple EdgeTypes and delegates instantiation to those ConnectionMappings.
% \item JoinedConnectionMapping: This operator represents a ConnectionMapping actually consisting of a path of ConnectionMappings. While acting as ConnectionMapping between two types within the specified path, it creates edges for each pair of nodes being connected through the whole path.
% \item MovedAttribute: This operator adds the value of an attribute of a node to nodes connected over a specified ConnectionMapping.
% \end{itemize}
