...
public class MetamodelImpl implements MutableMetamodel {
	...
	private Map<String, MutableJunctionNodeType> junctionNodeTypes = Maps
			.newHashMap();
	...
	protected MetamodelImpl(boolean createRootTypes) {
		if (createRootTypes) {
			...
			edge.createAttribute(edge.getDefaultAttributeGroup(),
					PSSIFConstants.BUILTIN_ATTRIBUTE_DIRECTED,
					PrimitiveDataType.BOOLEAN, true, AttributeCategory.METADATA);
			edge.createMapping(node, node);
		}
	}

	private final void addDefaultAttributes(MutableElementType type) {
		type.createAttribute(type.getDefaultAttributeGroup(),
				PSSIFConstants.BUILTIN_ATTRIBUTE_ID, PrimitiveDataType.STRING,
				true, AttributeCategory.METADATA);
		...
		type.createAttribute(type.getDefaultAttributeGroup(),
				...
				PrimitiveDataType.DATE, true, AttributeCategory.TIME);
		type.createAttribute(type.getDefaultAttributeGroup(),
		...
		type.createAttribute(type.getDefaultAttributeGroup(),
				"lowerCardinalitySource", PrimitiveDataType.STRING, true,
				AttributeCategory.METADATA);
		type.createAttribute(type.getDefaultAttributeGroup(),
				"upperCardinalitySource", PrimitiveDataType.STRING, true,
				AttributeCategory.METADATA);
		type.createAttribute(type.getDefaultAttributeGroup(),
				"lowerCardinalityDestination", PrimitiveDataType.STRING, true,
				AttributeCategory.METADATA);
		type.createAttribute(type.getDefaultAttributeGroup(),
				"upperCardinalityDestination", PrimitiveDataType.STRING, true,
				AttributeCategory.METADATA);
		type.createAttribute(type.getDefaultAttributeGroup(),
				"aggregation", PrimitiveDataType.STRING, true,
				AttributeCategory.METADATA);
	...
	public MutableEnumeration createEnumeration(String name) {
		...
		if (!getDataType(name).isNone()) {
			throw new PSSIFStructuralIntegrityException(
					"a datatype with the name " + name + " already exists");
		}
	...
	public PSSIFOption<Enumeration> getEnumeration(String name) {
		return PSSIFOption.<Enumeration> one(enumerations.get(PSSIFUtil
				.normalize(name)));
	}
	...
	public Collection<DataType> getDataTypes() {
		Collection<DataType> result = Sets.<DataType> newHashSet(enumerations
				.values());
	...
	public MutableNodeType createNodeType(String name) {
		...
		result.inherit(getMutableNodeType(PSSIFConstants.ROOT_NODE_TYPE_NAME)
				.getOne());
		return result;
	}
	...
	private MutableNodeType createNodeTypeInternal(String name) {
		...
		if (!getBaseNodeType(name).isNone()) {
			throw new PSSIFStructuralIntegrityException(
					"a nodetype with name '" + name + "' already exists");
		}
	...
	public MutableEdgeType createEdgeType(String name) {
		...
		result.inherit(getMutableEdgeType(PSSIFConstants.ROOT_EDGE_TYPE_NAME)
				.getOne());
		return result;
	}
	...
	private MutableEdgeType createEdgeTypeInternal(String name) {
		...
		if (!getEdgeType(name).isNone()) {
			throw new PSSIFStructuralIntegrityException(
					"an edgetype with name '" + name + "' already exists");
		}
	...
	public Collection<NodeTypeBase> getBaseNodeTypes() {
		Collection<NodeTypeBase> result = Sets
				.<NodeTypeBase> newHashSet(getNodeTypes());
	...
	public Collection<JunctionNodeType> getJunctionNodeTypes() {
		return ImmutableSet
				.<JunctionNodeType> copyOf(getMutableJunctionNodeTypes());
	}
	...
	public PSSIFOption<NodeTypeBase> getBaseNodeType(String name) {
		return PSSIFOption.<NodeTypeBase> merge(getMutableNodeType(name),
				getMutableJunctionNodeType(name));
	}
	...
	public PSSIFOption<JunctionNodeType> getJunctionNodeType(String name) {
		return PSSIFOption.<JunctionNodeType> many(getMutableJunctionNodeType(
				name).getMany());
	}

	@Override
	public PSSIFOption<EdgeType> getEdgeType(String name) {
		return PSSIFOption.<EdgeType> one(edgeTypes.get(PSSIFUtil
				.normalize(name)));
	}

	@Override
	public Collection<MutableNodeTypeBase> getMutableBaseNodeTypes() {
		Collection<MutableNodeTypeBase> result = Sets
				.<MutableNodeTypeBase> newHashSet(getMutableNodeTypes());
	...
	public PSSIFOption<MutableNodeTypeBase> getMutableBaseNodeType(String name) {
		return PSSIFOption.merge(getMutableNodeType(name),
				getMutableJunctionNodeType(name));
	}

	@Override
	public PSSIFOption<MutableJunctionNodeType> getMutableJunctionNodeType(
			String name) {
		return PSSIFOption
				.one(junctionNodeTypes.get(PSSIFUtil.normalize(name)));
	}
	...
	public void removeJunctionNodeType(JunctionNodeType junctionNodeType) {
		for (JunctionNodeType jnt : getJunctionNodeType(
				junctionNodeType.getName()).getMany()) {
